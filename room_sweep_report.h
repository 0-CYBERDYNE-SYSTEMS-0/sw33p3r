...
static inline size_t room_sweep_report_format(
    ...
    size_t used = 0;
    uint8_t all_sensors = (uint8_t)(RoomSweepReportSensorRf |
    ...
    room_sweep_report_append(
        ...
        state->sensors_confirmed == all_sensors ? "FULL" : "PARTIAL");
    ...
    room_sweep_report_append(
        ...
        capacity,
        &used,
        "Limitations: RSSI is not distance; wireless evidence does not prove Internet telemetry, recording, ownership, or intent; no observation does not prove absence; bounded carrier is not replay.\n");
    return used;
}
